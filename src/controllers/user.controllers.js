...
const registerUser = asyncHandler(async (req, res) => {
  ...
  //req.files is a property added to the req object by multer
  //avatar is an array of objects and we are accesing the firsst object using .
...
const refreshAccessToken = asyncHandler(async (req, res) => {
  //the user is sending token to us
  const incomingRefreshToken = req.cookie.refreshToken || req.body.refreshToken;

  if (!incomingRefreshToken) {
    throw new ApiError(401, "unauthorized request");
  }
  //now we will check whether the token that the user had and which was stored in the db are same or not
  try {
